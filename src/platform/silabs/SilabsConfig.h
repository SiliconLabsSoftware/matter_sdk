...
{
    return SL_TOKEN_GET_DYNAMIC_TOKEN(kMatterNvm3KeyDomain | static_cast<uint32_t>(keyBaseOffset) << 8 | id, 0);
}

constexpr inline uint32_t SilabsSecureTokenKey(uint8_t id)
{
    return SL_TOKEN_GET_STATIC_SECURE_TOKEN(kMatterStaticKeyDomain | id);
}
#else
...
public:
    ...
    // TODO : With the new CTM, we don't need these keys anymore as they will also be written and accessible through tokens
...
#ifdef SL_TOKEN_MANAGER_DEFINES_H
    static constexpr kConfigKey_Creds_Dac = SilabsSecureTokenKey(0x00);
    static constexpr kConfigKey_Creds_Pai = SilabsSecureTokenKey(0x32);
    static constexpr kConfigKey_Creds_CD  = SilabsSecureTokenKey(0x33);
#endif
    // Matter Config Keys
