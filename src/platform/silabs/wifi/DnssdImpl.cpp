...
CHIP_ERROR ChipDnssdInit(DnssdAsyncReturnCallback initCallback, DnssdAsyncReturnCallback errorCallback, void * context)
{
    ...
    {
        ChipLogError(DeviceLayer, "Failed to initialize mDNS: %x", status);
        if (errorCallback)
        {
...
#define MDNS_HOST_NAME       "wiseconnect1.local."
#define MDNS_SERVICE_TYPE    "_https._tcp.local."
...
{
    sl_mdns_t mdns;
    // sl_mdns_service_t service1      = { .instance_name   = MDNS_SERVICE_NAME,
...
// sl_status_t status1 = sl_mdns_register_service(&mdns, SL_NET_WIFI_CLIENT_INTERFACE, &service1);
...
// osDelay(10000);
    ...
    char instance_name[256]; // Allocate enough memory for the concatenated string
    ...
    if (service->mTextEntries && service->mTextEntrySize > 0)
    {
        ...
        for (size_t i = 0; i < service->mTextEntrySize; i++)
        {
            ...
            SILABS_LOG("KEY = %s      :::::     Value = %s", service->mTextEntries[i].mKey, reinterpret_cast<const char *>(service->mTextEntries[i].mData));
            ...
            // mdnsService.service_message = "\"" + service->mTextEntries[i].mKey + "=" + reinterpret_cast<const char *>(service->mTextEntries[i].mData) + "\" ";
        ...
        // mdnsService.service_message = "VP=65521+32774 SII=500 SAI=2000 SAT=4000 D=3840 CM=1 RI=0F00943BDE7E28778932B405DE876B6B0F4C PH=33";
        mdnsService.service_message = strdup(serviceMessage.c_str()); // Duplicate the string to ensure memory safety
        // mdnsService.service_message = "test12345678";
    }

//     SILABS_LOG("instance_name = %s, service_type = %s, port = %d", service->mName, service->mType, service->mPort);
// sl_mdns_service_t mdnsService      = { .instance_name   = "CD008F81E89E5658",
//     .service_type    = MDNS_SERVICE_TYPE,
//     .service_message = MDNS_SERVICE_MESSAGE,
//     .port            = MDNS_SERVICE_PORT,
//     .ttl             = MDNS_SERVICE_TTL };
