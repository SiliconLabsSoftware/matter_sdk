...
void SLApplyWiFiDeviceConfiguration(sl_wifi_device_configuration_t * configuration)
{
...
#if defined(SL_MATTER_ENABLE_DUAL_STACK) && SL_MATTER_ENABLE_DUAL_STACK
    // Enable basic SSL support for dual stack
    ...
    // Enable dual mode support for dual stack and SSL memory cloud support with more RAM for certificate storage
    configuration->boot_config.ext_tcp_ip_feature_bit_map |=
