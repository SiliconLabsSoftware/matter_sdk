...
/**
 * @brief rs911x/SiWx917 implementation of BlePlatformInterface
 *
 * This class wraps the RSI BLE stack for rs911x and SiWx917 platforms,
 * providing a unified interface for BLE operations.
 */
class BlePlatformRs911x : public BlePlatformInterface
{
public:
    BlePlatformRs911x();
    ~BlePlatformRs911x() override = default;
     ...
     */
    static BlePlatformRs911x & GetInstance();
...
private:
    ...
    static constexpr uint8_t kDefaultConnectionHandle = 1; // RSI BLE uses fixed connection handle
    ...
    // Helper methods
    CHIP_ERROR MapRSIError(int32_t status);
};
