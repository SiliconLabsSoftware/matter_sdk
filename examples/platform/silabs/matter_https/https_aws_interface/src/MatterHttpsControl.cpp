...
#include "MatterHttpsControl.h"

#ifdef SL_MATTER_ENABLE_HTTPS_OTA_FEAT
extern "C" {
extern void https_ota_init(void);
}
#endif
...
namespace matterHttps {
namespace control {
/*
 * This function received data from matterHttps mqtt. The data received is expected to be a
...
#ifdef SL_MATTER_ENABLE_HTTPS_OTA_FEAT
void https_ota_init_task(void * parameters)
{
    (void) parameters;
    https_ota_init();
}
...
void MatterHttpsIncomingDataCb(void * arg, const char * topic, uint16_t topic_len, const uint8_t * data, uint16_t len, uint8_t flags)
{
    ...
    ChipLogDetail(AppServer, "[MATTER_HTTPS] _value: %ld", _value);

#ifdef SL_MATTER_ENABLE_HTTPS_OTA_FEAT
    if (strcmp(cmd, "ota") == 0)
    {
        static TaskHandle_t HTTPS_OTA = NULL;
        if (HTTPS_OTA == NULL)
        {
            if ((pdPASS !=
                 xTaskCreate(https_ota_init_task, "HTTPS_OTA", HTTPS_OTA_TASK_STACK_SIZE, NULL, HTTPS_OTA_TASK_PRIORITY, &HTTPS_OTA)) ||
                !HTTPS_OTA)
            {
                ChipLogError(AppServer, "[MATTER_HTTPS] failed to create AWS OTA task");
                return;
            }
            ChipLogError(AppServer, "[MATTER_HTTPS] Task creation successfull for AWS OTA thread");
        }
        return;
    }
#endif
    ...
    if (cmdIndex == kStringNotFound)
    {
        ChipLogError(AppServer, "[MATTER_HTTPS] command %s not found", cmd);
    }
}

void SubscribeMQTT(intptr_t context)
{
    MatterHttpsMqttSubscribe(NULL, NULL, MatterHttpsIncomingDataCb, MQTT_SUBSCRIBE_TOPIC, MQTT_QOS_0);
}
...
void AttributeHandler(EndpointId endpointId, AttributeId attributeId)
{
    ...
    case ThermAttr::LocalTemperature::Id: {
        ...
        MatterHttpsSendMsg("LocalTemperature/Temp", (const char *) (buffer));
    }
    break;
    ...
    case ThermAttr::OccupiedCoolingSetpoint::Id: {
        ...
        MatterHttpsSendMsg("OccupiedCoolingSetpoint/coolingTemp", (const char *) (buffer));
    }
    break;
    ...
    case ThermAttr::OccupiedHeatingSetpoint::Id: {
        ...
        MatterHttpsSendMsg("OccupiedHeatingSetpoint/heatingTemp", (const char *) (buffer));
    }
    break;
    ...
    case ThermAttr::SystemMode::Id: {
        ...
        MatterHttpsSendMsg("thermostat/systemMode", Mode);
        MatterHttpsSendMsg("thermostat/currentTemp", (const char *) (buffer));
    }
    break;

    default: {
        ChipLogError(AppServer, "[MATTER_HTTPS] unhandled thermostat attributeId: %lx", attributeId);
        return;
    }
    break;
    }
