...
#if SL_MATTER_HTTPS_NVM_EMBED_CERT
    status =
...
#else
    *bufSize = sizeof(MATTER_HTTPS_SERVER_HOST);
    if (buf == NULL || buf_len < *bufSize)
    {
        return CHIP_ERROR_INTERNAL;
    }
    strncpy(buf, MATTER_HTTPS_SERVER_HOST, *bufSize);
#endif
    return status;
}
...
#else
    *bufSize = sizeof(MATTER_HTTPS_CLIENT_ID);
    if (buf == NULL || buf_len < *bufSize)
    {
        return CHIP_ERROR_INTERNAL;
    }
    strncpy(buf, MATTER_HTTPS_CLIENT_ID, *bufSize);
#endif
    return status;
}
