...
-   [Matter EFR32 Fan Control Example](#matter-efr32-fan-control-example)
...
Fan-control-specific override hooks include `InitFanControlImpl()`,
`HandleStepImpl()`, `FanModeWriteCallbackImpl()`,
`PercentSettingWriteCallbackImpl()`, and `SpeedSettingWriteCallbackImpl()`.
...
2. Declare the same method signature in `CustomerAppTask` in your
   ...
   even when the public dispatcher (for example `ButtonEventHandler`) is
   `static`.
